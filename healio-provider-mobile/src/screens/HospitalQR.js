...
export default function HospitalQR({ navigation }) {
  const { user } = useAuth();
      ...
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false} contentContainerStyle={{ alignItems: 'center' }}>
        <View style={styles.qrCard}>
          <View style={styles.qrTop}>
            ...
            <View>
              <Text style={styles.qrHospital}>{hospitalName}</Text>
              <Text style={styles.qrLicense}>Hospital</Text>
            </View>
          </View>
          <View style={styles.qrFrame}>
            {hospitalId
              ? <QRCode value={`healio:hospital:${hospitalId}`} size={220} color={COLORS.primary} backgroundColor={COLORS.white} />
              : <Text style={styles.qrMissing}>Hospital identity unavailable</Text>}
          </View>
          <Text style={styles.qrCaption}>Healio Hospital Code</Text>
        </View>

        <TouchableOpacity style={styles.pendingBtn} onPress={() => navigation.navigate('Patients')}>
          <Ionicons name="people-outline" size={18} color={COLORS.white} />
          <Text style={styles.pendingBtnText}>Go to Patients</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  qrHospital: { fontSize: 16, fontWeight: '800', color: COLORS.text },
