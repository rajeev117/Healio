...
import { fetchPharmacyPatients, fetchPharmacyEarnings, fetchOrgInfo } from '../services/api';
...
export default function Home({ navigation }) {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    try {
      const [pts, earn, orgInfo] = await Promise.all([
        fetchPharmacyPatients(hospitalId),
      ...
      console.warn('Pharmacy Home load error:', e.message);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
  ...
  // Patient prescription orders waiting for a pharmacist to price them.
      ...
      >
          ...
          <View style={styles.headerTop}>
            <TouchableOpacity style={styles.avatarBtn} onPress={() => navigation.navigate('Profile')}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.roleLine}>Pharmacy</Text>
              </View>
            </TouchableOpacity>
        ...
        <View style={styles.quickSection}>
          ...
          >
            ...
            <View style={{ flex: 1 }}>
              <Text style={styles.requestsLabel}>Order requests</Text>
        ...
        <View style={styles.section}>
          <View style={styles.quickGrid}>
            <QuickAction icon="document-text" label="Prescriptions" bg={COLORS.tintTeal} color={COLORS.tintTealInk} onPress={() => navigation.navigate('Prescriptions')} />
            <QuickAction icon="wallet"         label="Earnings"      bg={COLORS.tintBlue} color={COLORS.tintBlueInk} onPress={() => navigation.navigate('Earnings')} />
            <QuickAction icon="qr-code"        label="My QR"         bg={COLORS.tintGold} color={COLORS.tintGoldInk} onPress={() => navigation.navigate('PharmacyQR')} />
        ...
        <View style={[styles.section, { marginBottom: SPACING.xl * 2 }]}>
          ...
          {patients.length === 0 && (
            <View style={styles.emptyBox}>
              <Ionicons name="medkit-outline" size={36} color={COLORS.borderStrong} />
              <Text style={styles.emptyText}>No pharmacy orders yet</Text>
