...
import { fetchPharmacyPatients } from '../services/api';
...
export default function Prescriptions({ navigation }) {
  ...
  const load = useCallback(async (isRefresh = false) => {
    ...
    try {
      const data = await fetchPharmacyPatients(hospitalId);
      setPatients(data);
    } catch (e) {
      console.warn('Pharmacy Prescriptions error:', e.message);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      <View style={styles.header}>
        <View style={styles.headerTop}>
          <View>
            <Text style={styles.greeting}>Patient Prescriptions</Text>
            <Text style={styles.pharmacyName}>{hospitalName}</Text>
          </View>
            ...
            >
              ...
              <View style={styles.cardRight}>
                <View style={styles.countPill}>
                  <Text style={styles.countText}>{p.orders.length} Rx</Text>
                </View>
...
const styles = StyleSheet.create({
  ...
  pharmacyName: { color: COLORS.white, fontSize: 20, fontWeight: '800', marginTop: 2, letterSpacing: -0.3 },
  ordersBadge: {
