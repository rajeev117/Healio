...
// org. An independent lab never has appointments, so it would always report
// "no visits found". Here the scan resolves identity and goes straight to the
// quote.
//
// Note the asymmetry with the lab's own QR: resolve_qr_token gives us the
// patient's identity but does not create a qr_checkins row, so it does not
// unlock their prescriptions. For those, the patient has to scan the lab's code
// — which is what RxPreview's empty state asks for.
...
export default function ScanWalkIn({ navigation }) {
  ...
  const onScanned = useCallback(async ({ data }) => {
    ...
    try {
      ...
      navigation.replace('TestRequestQuote', {
    ...
    <SafeAreaView style={styles.safe} edges={['top']}>
      ...
      <View style={[styles.footer, { marginBottom: 20 + insets.bottom }]}>
        ...
        <Text style={styles.footerText}>
          To see the patient's prescriptions, ask them to scan your lab QR instead — that is what shares them.
        </Text>
      </View>
    </SafeAreaView>
  );
