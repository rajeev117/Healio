// The lab's counter QR.
//
// The payload is `healio:lab:<organisationId>`, exactly what resolve_provider_qr
// (migration-043) already resolves — it accepts kind 'lab' and looks the id up in
// `organisations` without checking the org's type. So an independent lab's code
// works through the patient app's existing scan → check-in path with no change
// on either side. The check-in is also what unlocks the patient's prescriptions
// for this lab, via checkin_prescriptions.
      ...
      >
        <View style={styles.qrCard}>
          <View style={styles.qrTop}>
            <View style={styles.logoBox}><Ionicons name="flask" size={18} color={COLORS.white} /></View>
            <View style={{ flex: 1 }}>
              <Text style={styles.qrLab} numberOfLines={1}>{hospitalName}</Text>
              <Text style={styles.qrKind}>Independent diagnostic lab</Text>
            </View>
          </View>

          <View style={styles.qrFrame}>
            <QRCode value={`healio:lab:${orgId}`} size={220} color={COLORS.primary} backgroundColor={COLORS.white} />
          </View>
        ...
        <TouchableOpacity style={styles.cta} onPress={() => navigation.navigate('Main', { screen: 'Requests' })}>
          <Ionicons name="flask-outline" size={18} color={COLORS.white} />
          <Text style={styles.ctaText}>Go to lab orders</Text>
        </TouchableOpacity>
