// ─────────────────────────────────────────────────────────────────────────────
// PrescriptionRequest — "tell the provider what the patient needs" step, shared
// by the RMP's pharmacy order and lab booking flows.
//
// Same contract as the patient app's component (roles/patient/components): the
// RMP either attaches the patient's prescription (photo, gallery image or PDF),
// types the details as free text, or both — the parent screen gates Continue on
// `hasRequest()`, which requires at least one of the two. Styled with the RMP
// palette so the flow reads as part of this module.
// ─────────────────────────────────────────────────────────────────────────────
...
export default function PrescriptionRequest({
  ...
  subtitle = 'Photograph the patient’s prescription or attach a PDF.',
  notesLabel = 'Details or message',
  notesPlaceholder = 'Type what the patient needs, or add a note for the provider…',
    ...
    try {
      ...
      if (perm.status !== 'granted') {
        Alert.alert(
          'Permission needed',
          fromCamera
            ? 'Allow camera access to photograph the prescription.'
            : 'Allow photo library access to attach the prescription.'
        );
        return;
      }
...
const styles = StyleSheet.create({
  ...
  notesInput: {
    ...
    fontSize: 14, lineHeight: 20, color: COLORS.text, backgroundColor: COLORS.surface,
  },
