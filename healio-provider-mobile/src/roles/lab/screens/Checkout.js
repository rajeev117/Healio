...
import { completeLabOrder } from '../services/api';
...
export default function Checkout({ route, navigation }) {
  ...
  const [bill, setBill]       = useState(null);   // receipt — { uri, mimeType }
  const [report, setReport]   = useState(null);   // test result — { uri, mimeType }
  const [loading, setLoading] = useState(false);

  const pickFromGallery = async (setFile) => {
    const permission = await ImagePicker.requestMediaLibraryPermissionsAsync();
    if (!permission.granted) {
      Alert.alert('Permission required', 'Allow photo access to attach the file.');
      return;
    }
  ...
  const takePhoto = async (setFile) => {
    const permission = await ImagePicker.requestCameraPermissionsAsync();
    if (!permission.granted) {
      Alert.alert('Permission required', 'Allow camera access to capture the file.');
      return;
    }
  ...
  const chooseSource = (setFile, title = 'Upload') => {
    Alert.alert(title, 'Choose a source', [
      { text: 'Take Photo', onPress: () => takePhoto(setFile) },
      { text: 'Choose from Gallery', onPress: () => pickFromGallery(setFile) },
      { text: 'Cancel', style: 'cancel' },
    ]);
  };

  // Both the bill and the report use the same preview / picker UI.
  const renderUpload = ({ file, setFile, title, hint, icon, chooserTitle }) => (
    file ? (
      <View style={styles.billPreview}>
        <Image source={{ uri: file.uri }} style={styles.billImage} resizeMode="cover" />
        <View style={styles.billActions}>
          <TouchableOpacity style={styles.billActionBtn} onPress={() => chooseSource(setFile, chooserTitle)}>
            <Ionicons name="swap-horizontal" size={16} color={COLORS.primary} />
            <Text style={styles.billActionText}>Replace</Text>
          </TouchableOpacity>
          <TouchableOpacity style={styles.billActionBtn} onPress={() => setFile(null)}>
      ...
      <TouchableOpacity style={styles.uploadCard} onPress={() => chooseSource(setFile, chooserTitle)} activeOpacity={0.8}>
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const reportUrl = report ? await uploadRecordImage(report.uri, patient?.id) : null;
      await completeLabOrder(patient.id, hospitalId, amt, billUrl, reportUrl);
      Alert.alert(
        'Checkout complete',
        `₹${amt} billed to ${patient?.name}. Bill uploaded${reportUrl ? ' · report shared with patient' : ''}.`,
      ...
      >
        <ScrollView style={styles.container} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          ...
          {renderUpload({
            ...
            setFile: setBill,
            title: 'Upload bill',
            hint: 'Take a photo or choose from your gallery',
            icon: 'receipt-outline',
            chooserTitle: 'Upload Bill',
          })}

          <Text style={styles.label}>Lab Report<Text style={styles.optional}> (optional)</Text></Text>
          {renderUpload({
            file: report,
            setFile: setReport,
            title: 'Upload report',
            hint: 'Attach the test result to share it with the patient',
            icon: 'document-text-outline',
            chooserTitle: 'Upload Report',
          })}
...
const styles = StyleSheet.create({
  ...
  optional: { color: COLORS.textSecondary, fontWeight: '600' },
  amountRow: {
